...
class GMMParameter(Structure):
    _fields_ = [("nr_instance", c_int), ("nr_dim", c_int), ("nr_mixture", c_int), ("min_covar", c_double), ("threshold", c_double), ("nr_iteration", c_int),
                ("init_with_kmeans", c_int), ("concurrency", c_int), ("verbosity", c_int)]
...
class GMM:
    ...
    def score_all(self, X):
        X_c = self._double_array_python_to_ctype(X)
        param = self._gen_param(X)
        return pygmm.score_all(self.gmm, X_c, param.nr_instance, param.nr_dim, param.concurrency)
